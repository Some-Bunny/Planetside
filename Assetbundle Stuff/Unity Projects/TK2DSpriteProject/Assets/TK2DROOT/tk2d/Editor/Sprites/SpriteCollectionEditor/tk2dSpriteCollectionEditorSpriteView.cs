...
namespace tk2dEditor.SpriteCollectionEditor
{
	public class SpriteView
	{
		...
		public void DrawSpriteEditorInspector(List<SpriteCollectionEditorEntry> entries, bool allowDelete, bool editingSpriteSheet)
		{
			...
			param.metadata.type = (TilesetIndexMetadata.TilesetFlagType)EditorGUILayout.EnumPopup("Type", param.metadata.type);
			//
			HandleMultiSelection(entries, (a, b) => a.metadata.type == b.metadata.type, (a, b) => b.metadata.type = a.metadata.type);

			// metadata amount
			param.metadata.weight = (float)EditorGUILayout.FloatField("Weight", param.metadata.weight);
			HandleMultiSelection(entries, (a, b) => a.metadata.weight == b.metadata.weight, (a, b) => b.metadata.weight = a.metadata.weight);
