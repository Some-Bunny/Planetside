    ...
    class MagnetBehaviour : MonoBehaviour
    {
        ...
        private bool AdjustRigidbodyVelocity(SpeculativeRigidbody other)
        {
            ...
            if (num < radiusSquared)
            {
                ...
                if (debugMode) ETGModConsole.Log("Rigidbody was valid");
                Vector2 frameAccelerationForRigidbody = this.GetFrameAccelerationForRigidbody(other.UnitCenter, Mathf.Sqrt(num), gravitationalForce);
                float d = Mathf.Clamp(BraveTime.DeltaTime, 0f, 0.02f);
                ...
                other.aiActor.specRigidbody.Velocity += vector;
                return true;
            }
            return false;
        }
