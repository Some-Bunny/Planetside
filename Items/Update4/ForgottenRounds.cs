...
namespace Planetside
{
	public class ForgottenRoundOubliette : BasicStatPickup
	{
        ...
        public static void OnMasteryDetermineContents(RewardPedestal pedestal, PlayerController determiner, CustomActions.ValidPedestalContents valids)
        {
            if (pedestal.ContainsMasteryTokenForCurrentLevel())
            {
                ...
                {
                    valids.overrideItemPool.Add(new Tuple<int, float>(ForgottenRoundOubliette.ForgottenRoundOublietteID, 1.5f));
                }
            }
        }
	...
	public class ForgottenRoundAbbey : BasicStatPickup
	{
        ...
        public static void OnMasteryDetermineContents(RewardPedestal pedestal, PlayerController determiner, CustomActions.ValidPedestalContents valids)
        {
            if (pedestal.ContainsMasteryTokenForCurrentLevel())
            {
                ...
                {
                    valids.overrideItemPool.Add(new Tuple<int, float>(ForgottenRoundAbbey.ForgottenRoundAbbeyID, 1.5f));
                }
            }
        }
    ...
    public class ForgottenRoundRat: BasicStatPickup
    {
        public static void Init()
        {
            ...
            string longDesc = "Increases damage. This exceptional, yet false artifact indicates mastery of the third hidden chamber.\n\nAll the power of this forgotten round comes from the piece of cheese adorned atop it.";
            ItemBuilder.SetupItem(warVase, shortDesc, longDesc, "psog", "Forgotten Round");
            warVase.quality = PickupObject.ItemQuality.SPECIAL;
            warVase.IsMasteryToken = true;
            warVase.ForcedPositionInAmmonomicon = 4;
    ...
    [HarmonyPatch(typeof(SellCellController), nameof(SellCellController.HandleSoldItem))]
    public class Patch_Fuck
    {
        [HarmonyPrefix]
        //this is a stupid fucking fix but I am tired as shit with everything so itll do
        private static void Awake(SellCellController __instance, PickupObject targetItem)
        {
            if (targetItem.PickupObjectId == ForgottenRoundRNG.ForgottenRoundRNGID 
                || targetItem.PickupObjectId == ForgottenRoundAbbey.ForgottenRoundAbbeyID 
                || targetItem.PickupObjectId == ForgottenRoundRat.ForgottenRoundRatID 
                || targetItem.PickupObjectId == ForgottenRoundOubliette.ForgottenRoundOublietteID)
            {
                __instance.m_masteryRoundsSold++;
            }
        }
    }
    /*
    [HarmonyPatch]
    public static class FixTheStupidMaybeThing
    {
