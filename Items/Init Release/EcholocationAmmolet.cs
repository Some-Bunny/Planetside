...
namespace Planetside
{
    public class EcholocationAmmolet : BlankModificationItem
    {
        ...
        private static Hook BlankHook = new Hook(typeof(SilencerInstance).GetMethod("ProcessBlankModificationItemAdditionalEffects", BindingFlags.Instance | BindingFlags.NonPublic), typeof(EcholocationAmmolet).GetMethod("BlankModHook", BindingFlags.Instance | BindingFlags.Public), typeof(SilencerInstance));
        ...
        public void BlankModHook(Action<SilencerInstance, BlankModificationItem, Vector2, PlayerController> orig, SilencerInstance silencer, BlankModificationItem bmi, Vector2 centerPoint, PlayerController user)
        {
            ...
            try
            {
                ...
                {                    
                    ...
                    AkSoundEngine.PostEvent("Play_CHR_bat_death_01", user.gameObject);
                    ParticleBase.EmitParticles("WaveParticle", 1, new ParticleSystem.EmitParams()
                    {
                        ...
                        rotation = 0,
                        startLifetime = 0.5f,
                        startColor = new Color(0, 0.1f, 0.4f, 0.05f)
                    });
                }
            }
            catch (Exception e)
